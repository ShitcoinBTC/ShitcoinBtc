...
public:
    ...
    //! Total native vault yield minted so far (0 if unsupported).
    virtual CAmount GetVaultYieldTotal() const { return 0; }
    //! Native vault yield minted in schedule-year nYear (0 if unsupported).
    virtual CAmount GetVaultYieldYear(int32_t nYear) const { (void)nYear; return 0; }
    //! Stage vault yield counters into this view (default: ignore).
    //! nTotal/fTotalDirty carry the absolute total when dirty; mapYears
    //! carries absolute per-year values to merge.
    virtual void PushVaultCounters(CAmount nTotal, bool fTotalDirty, const std::map<int32_t, CAmount>& mapYears) { (void)nTotal; (void)fTotalDirty; (void)mapYears; }
};
...
public:
    ...
    CAmount GetVaultYieldTotal() const override;
    CAmount GetVaultYieldYear(int32_t nYear) const override;
    void PushVaultCounters(CAmount nTotal, bool fTotalDirty, const std::map<int32_t, CAmount>& mapYears) override;
};
...
private:
    ...
    /**
     * Staged native vault yield counters. Mutable so the const getters can
     * serve staged values; pushed to the base view via PushVaultCounters()
     * in Flush()/Sync() and cleared once the base write succeeds.
     */
    mutable std::optional<CAmount> m_stagedVaultTotal;
    mutable std::map<int32_t, CAmount> m_stagedVaultYears;
    mutable std::set<int32_t> m_dirtyVaultYears;
    mutable bool m_vaultTotalDirty{false};
...
public:
    ...
    //! Native vault yield counters (see CheckVaultYield in validation.cpp).
    CAmount GetVaultYieldTotal() const override;
    CAmount GetVaultYieldYear(int32_t nYear) const override;
    void PushVaultCounters(CAmount nTotal, bool fTotalDirty, const std::map<int32_t, CAmount>& mapYears) override;
    /**
     * Stage a vault-yield counter delta for schedule-year nYear (negative
     * when disconnecting a block). The delta is applied on top of the
     * currently visible counter value and pushed to the base view on the
     * next Flush()/Sync().
     */
    void StageVaultYield(int32_t nYear, CAmount nDelta);
