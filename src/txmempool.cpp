...
void CTxMemPool::check(const CCoinsViewCache& active_coins_tip, int64_t spendheight) const
{
    ...
    for (const auto& it : GetSortedDepthAndScore()) {
        ...
        // Native vault claims mint yield, so value_out may exceed value_in by the
        // entitlement; CheckTxInputs without the mint would fail for them. The
        // mint itself is validated by CheckVaultYield on acceptance.
        bool fHasVaultInput = false;
        for (const CTxIn& txin : tx.vin) {
            const auto& witStack = txin.scriptWitness.stack;
            if (witStack.empty()) continue;
            CScript candidate(witStack.back().begin(), witStack.back().end());
            uint32_t nLockTime{0};
            CPubKey vaultPubKey;
            if (IsVaultLockScript(candidate, nLockTime, vaultPubKey)) { fHasVaultInput = true; break; }
        }
        if(!bFoundConflict && !fHasVaultInput)
