...
/**
 * Native vault yield check (consensus).
 *
 * A transaction spending a native vault lock (P2WSH of BuildVaultLockScript,
 * revealed as the last witness stack item) may mint new coins as vault
 * yield, drawn from the protocol-level nVaultReserveTotal. The entitlement
 * for each vault input is derived from the lock duration (spend
 * median-time-past versus the lock's confirmation median-time-past) mapped
 * onto the configured APY tiers; regtest zeroes the reserve, disabling
 * yield entirely.
 *
 * On success returns true and sets nMintedOut to the amount of new coins
 * minted by this transaction (outputs minus inputs, floored at zero).
 * nSpendHeight is currently unused (tiers are MTP-based) but kept for
 * future schedule rules.
 */
static bool CheckVaultYield(const CTransaction& tx, TxValidationState& state, const CCoinsViewCache& view, const CChain& chain, int nSpendHeight, int64_t nSpendMTP, const Consensus::Params& consensusParams, CAmount& nMintedOut)
{
    (void)nSpendHeight;
    nMintedOut = 0;
    CAmount nTotalEntitlement{0};
    bool fHasVaultInput{false};

    for (size_t i = 0; i < tx.vin.size(); ++i) {
        const Coin& coin = view.AccessCoin(tx.vin[i].prevout);
        if (coin.IsSpent()) continue;
        const auto& witStack = tx.vin[i].scriptWitness.stack;
        if (witStack.empty()) continue;
        CScript candidate(witStack.back().begin(), witStack.back().end());
        uint32_t nLockTime{0};
        CPubKey pubkey;
        if (!IsVaultLockScript(candidate, nLockTime, pubkey)) continue;
        if (coin.out.scriptPubKey != GetVaultLockP2WSH(candidate)) continue;

        // This input spends a native vault lock: compute its yield entitlement.
        fHasVaultInput = true;
        const CAmount nPrincipal = coin.out.nValue;
        if (!MoneyRange(nPrincipal)) {
            return state.Invalid(TxValidationResult::TX_CONSENSUS, "bad-vault-principal-range",
                                 "vault input value out of range");
        }
        const CBlockIndex* pLockIndex = chain[coin.nHeight];
        if (!pLockIndex) {
            return state.Invalid(TxValidationResult::TX_CONSENSUS, "bad-vault-lock-height",
        ...
        if (consensusParams.nVaultReserveTotal != 0) {
            ...
            if (nEntitlement < 0 || !MoneyRange(nEntitlement)) {
                return state.Invalid(TxValidationResult::TX_CONSENSUS, "bad-vault-entitlement-range",
                                     "vault yield entitlement out of range");
            }
        }
        nTotalEntitlement += nEntitlement;
        if (!MoneyRange(nTotalEntitlement)) {
            return state.Invalid(TxValidationResult::TX_CONSENSUS, "bad-vault-entitlement-range",
                                 "total vault yield entitlement out of range");
        }
    }

    if (!fHasVaultInput) return true;
    ...
    for (const CTxIn& txin : tx.vin) {
        ...
        if (!MoneyRange(nValueIn)) {
            return state.Invalid(TxValidationResult::TX_CONSENSUS, "bad-vault-input-range",
                                 "vault transaction input value out of range");
        }
    }
    CAmount nValueOut{0};
    for (const CTxOut& txout : tx.vout) {
        ...
        if (!MoneyRange(nValueOut)) {
            return state.Invalid(TxValidationResult::TX_CONSENSUS, "bad-vault-output-range",
                                 "vault transaction output value out of range");
        }
    }
    const CAmount nMinted = nValueOut > nValueIn ? nValueOut - nValueIn : 0;
    if (nMinted > nTotalEntitlement) {
        return state.Invalid(TxValidationResult::TX_CONSENSUS, "bad-vault-yield-exceeded",
                             strprintf("vault transaction mints %s exceeding entitlement %s",
                                       FormatMoney(nMinted), FormatMoney(nTotalEntitlement)));
    }
    nMintedOut = nMinted;
    return true;
}
...
bool MemPoolAccept::PreChecks(ATMPArgs& args, Workspace& ws)
{
    ...
    // Native vault yield check (consensus) runs before CheckTxInputs: a claim
    // transaction mints new coins up to its entitlement, which CheckTxInputs
    // must allow. No counter staging in the mempool.
    CAmount nVaultMinted{0};
    if (!CheckVaultYield(tx, state, m_view, m_active_chainstate.m_chain,
                         m_active_chainstate.m_chain.Height() + 1,
                         m_active_chainstate.m_chain.Tip()->GetMedianTimePast(),
                         args.m_chainparams.GetConsensus(), nVaultMinted)) {
        return false; // state filled in by CheckVaultYield
    }
    if (!Consensus::CheckTxInputs(tx, state, m_view, m_active_chainstate.m_chain.Height() + 1, ws.m_base_fees, mapAssetIn, mapAssetOut, nVaultMinted)) {
...
CAmount GetBlockSubsidy(unsigned int nHeight, const Consensus::Params& consensusParams, bool fSuperblockPartOnly)
{
    ...
    if (nHeight == 0)
        return 50*COIN; // genesis block reward, unspendable like Bitcoin's
    if (nHeight == 1)
    {
...
DisconnectResult Chainstate::DisconnectBlock(const CBlock& block, const CBlockIndex* pindex, CCoinsViewCache& view, NEVMMintTxSet &setMintTxs, std::vector<uint256> &vecNEVMBlocks, std::vector<std::pair<uint256, uint32_t> > &vecTXIDPairs, bool bReverify, bool bReplay)
{
    ...
    // Compute this block's native vault yield from the undo data before the
    // undo entries are moved-from in the loop below, so the vault counters
    // can be debited from the coins view on disconnect.
    CAmount nBlockVaultYield{0};
    for (size_t i = 1; i < block.vtx.size(); ++i) {
        const CTransaction& tx = *(block.vtx[i]);
        const CTxUndo& txundo = blockUndo.vtxundo[i - 1];
        if (txundo.vprevout.size() != tx.vin.size()) continue;
        CAmount nTxIn{0};
        CAmount nTxOut{0};
        bool fVaultInput{false};
        for (size_t j = 0; j < tx.vin.size(); ++j) {
            const Coin& undoCoin = txundo.vprevout[j];
            nTxIn += undoCoin.out.nValue;
            const auto& witStack = tx.vin[j].scriptWitness.stack;
            if (witStack.empty()) continue;
            CScript candidate(witStack.back().begin(), witStack.back().end());
            uint32_t nLockTime{0};
            CPubKey pubkey;
            if (!IsVaultLockScript(candidate, nLockTime, pubkey)) continue;
            if (undoCoin.out.scriptPubKey != GetVaultLockP2WSH(candidate)) continue;
            fVaultInput = true;
        }
        for (const CTxOut& txout : tx.vout) {
            nTxOut += txout.nValue;
        }
        if (!MoneyRange(nTxIn) || !MoneyRange(nTxOut)) continue;
        if (fVaultInput && nTxOut > nTxIn) {
            nBlockVaultYield += nTxOut - nTxIn;
            if (!MoneyRange(nBlockVaultYield)) {
                // Cannot happen for valid undo data; the loop below will
                // fail the disconnect on inconsistent data.
                nBlockVaultYield = 0;
                break;
            }
        }
    ...
    // Debit this block's vault yield from the coins view being disconnected;
    // it is pushed to the base view on Flush()/Sync() with the coin undos.
    if (nBlockVaultYield > 0) {
        const int32_t nVaultYear = pindex->nHeight / params.nSubsidyHalvingInterval;
        view.StageVaultYield(nVaultYear, -nBlockVaultYield);
    }
                  ...
                  NEVMMintTxSet &setMintTxs, NEVMTxRootMap &mapNEVMTxRoots, PoDAMAPMemory &mapPoDA, std::vector<std::pair<uint256, uint32_t> > &vecTXIDPairs, bool bReverify)
{
    ...
    CAmount nFees = 0;
    // Total native vault yield minted by this block's transactions.
    CAmount nBlockVaultYield{0};
    int nInputs = 0;
    ...
    for (unsigned int i = 0; i < block.vtx.size(); i++)
    {
        ...
        if (!tx.IsCoinBase())
        {
            ...
            CAssetsMap mapAssetIn;
            CAssetsMap mapAssetOut;
            // Native vault yield check (consensus) runs before CheckTxInputs: a
            // claim transaction mints new coins up to its entitlement, which
            // CheckTxInputs must allow via nTxVaultMinted.
            CAmount nTxVaultMinted{0};
            {
                TxValidationState tx_state_vault;
                if (!CheckVaultYield(tx, tx_state_vault, view, m_chain, pindex->nHeight, pindex->pprev->GetMedianTimePast(), params.GetConsensus(), nTxVaultMinted)) {
                    // Any transaction validation failure in ConnectBlock is a block consensus failure
                    state.Invalid(BlockValidationResult::BLOCK_CONSENSUS,
                                tx_state_vault.GetRejectReason(), tx_state_vault.GetDebugMessage());
                    return error("%s: CheckVaultYield: %s, %s", __func__, tx.GetHash().ToString(), state.ToString());
                }
                nBlockVaultYield += nTxVaultMinted;
                if (!MoneyRange(nBlockVaultYield)) {
                    return state.Invalid(BlockValidationResult::BLOCK_CONSENSUS, "bad-vault-yield-outofrange");
                }
            }
            if (!Consensus::CheckTxInputs(tx, tx_state, view, pindex->nHeight, txfee, mapAssetIn, mapAssetOut, nTxVaultMinted)) {
                // Any transaction validation failure in ConnectBlock is a block consensus failure
                state.Invalid(BlockValidationResult::BLOCK_CONSENSUS,
    ...
    // END SYSCOIN
    // Native vault yield schedule caps (consensus, enforced in fJustCheck mode too).
    if (nBlockVaultYield > 0) {
        const int32_t nVaultYear = pindex->nHeight / params.GetConsensus().nSubsidyHalvingInterval;
        const CAmount nYearMinted = view.GetVaultYieldYear(nVaultYear) + nBlockVaultYield;
        const CAmount nTotalMinted = view.GetVaultYieldTotal() + nBlockVaultYield;
        if (!MoneyRange(nYearMinted) || nYearMinted > params.GetConsensus().nVaultYearlyYieldCap) {
            return state.Invalid(BlockValidationResult::BLOCK_CONSENSUS, "bad-vault-yearly-cap",
                                 strprintf("vault yield %s in schedule-year %d exceeds yearly cap %s",
                                           FormatMoney(nBlockVaultYield), nVaultYear,
                                           FormatMoney(params.GetConsensus().nVaultYearlyYieldCap)));
        }
        if (!MoneyRange(nTotalMinted) || nTotalMinted > params.GetConsensus().nVaultReserveTotal) {
            return state.Invalid(BlockValidationResult::BLOCK_CONSENSUS, "bad-vault-reserve-exhausted",
                                 strprintf("vault yield %s exceeds reserve total %s",
                                           FormatMoney(nBlockVaultYield),
                                           FormatMoney(params.GetConsensus().nVaultReserveTotal)));
        }
    }
    ...
    // Stage this block's vault yield on the coins view being connected; it is
    // pushed to the base view on Flush()/Sync() and persisted atomically with
    // the coins in CCoinsViewDB::BatchWrite. Nothing fallible remains below.
    if (nBlockVaultYield > 0) {
        const int32_t nVaultYear = pindex->nHeight / params.GetConsensus().nSubsidyHalvingInterval;
        view.StageVaultYield(nVaultYear, nBlockVaultYield);
    }
 ...
 * template at launch; consensus pins the amounts so the 21B supply schedule
 * cannot be altered after the fact.
 */
static bool CheckAllocationBlock(const CBlock& block, BlockValidationState& state, const Consensus::Params& consensusParams)
{
    ...
    if (!std::equal(std::begin(expected), std::end(expected), std::begin(actual))) {
        return state.Invalid(BlockValidationResult::BLOCK_CONSENSUS, "bad-alloc-amounts",
                             "allocation block coinbase does not match the launch schedule");
    }
    return true;
}
...
static bool ContextualCheckBlock(const CBlock& block, BlockValidationState& state, const ChainstateManager& chainman, const CBlockIndex* pindexPrev)
{
    ...
    // Enforce the Shitcoin launch allocation schedule in block 1.
    // (Skipped where the launch allocations are zero, e.g. regtest.)
    if (nHeight == 1 && chainman.GetConsensus().nTeamAllocation > 0) {
        if (!CheckAllocationBlock(block, state, chainman.GetConsensus())) {
            return false;
        }
    }
...
bool Chainstate::RollforwardBlock(const CBlockIndex* pindex, CCoinsViewCache& inputs, NEVMTxRootMap &mapNEVMTxRoots, NEVMMintTxSet &setMintTxs, PoDAMAPMemory &mapPoDA, std::vector<std::pair<uint256, uint32_t> > &vecTXIDPairs)
{
    ...
    for (const CTransactionRef& tx : block.vtx) {
        ...
        if (!tx->IsCoinBase()) {
            ...
            // SYSCOIN
            const uint256& txHash = tx->GetHash();
            // Native vault yield check (consensus) runs before CheckTxInputs so
            // that claim transactions minting yield are allowed.
            CAmount nTxVaultMinted{0};
            {
                TxValidationState tx_state_vault;
                if (!CheckVaultYield(*tx, tx_state_vault, inputs, m_chain, pindex->nHeight, pindex->pprev ? pindex->pprev->GetMedianTimePast() : 0, chainParams, nTxVaultMinted)) {
                    return error("%s: CheckVaultYield: %s, %s", __func__, txHash.ToString(), tx_state_vault.ToString());
                }
            }
            if (!Consensus::CheckTxInputs(*tx, tx_state, inputs, pindex->nHeight, txfee, mapAssetIn, mapAssetOut, nTxVaultMinted)) {
